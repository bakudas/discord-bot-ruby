...
bot = Discordrb::Commands::CommandBot.new(token: ENV['DISCORD_TOKEN'], prefix: '!', intents: [:server_messages]) # initializing the bot
#bot = Discordrb::Bot.new(token: ENV['DISCORD_TOKEN'], intents: [:server_messages]) # initializing the bot

hora = Time.now.ctime

puts '-------------------------------------'
puts "Logado como: #{bot.name} às #{hora}"
puts '-------------------------------------'
...
bot.command :peitao do |event|
  bot.send_file(event.channel, File.open('app/data/images/babyelias.gif', 'r'))
end

bot.command :bongiorno do |event|
  bot.send_file(event.channel, File.open('app/data/images/bongiornogif.gif', 'r'))
end
...
bot.command :regras do |event|
  bot.send_file(event.channel, File.open("app/data/images/regras0#{rand(1...3)}.jpg", 'r'))
end
...
bot.commands.each { |symbol, _command, _command_alias| puts symbol }
